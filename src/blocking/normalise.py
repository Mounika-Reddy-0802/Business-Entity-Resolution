...
def normalise_frame(df):
    ...
    out = df.copy()
    out["name_clean"] = out["business_name"].map(fold)
    ...
    views = [addr_views(a, c) for a, c in zip(out["business_address"], out["country"])]
...
def main(splits):
    ...
    for split in splits:
        for s, df in load_sources(split).items():
            normalise_frame(df).to_parquet(out / f"{split}_{s}.parquet", index=False)
